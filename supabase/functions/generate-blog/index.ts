...
async function callDeepSeek(_: string | null, articles: NewsArticle[]): Promise<string> {
  const OPENROUTER_API_KEY = Deno.env.get("OPENROUTER_API_KEY");
  if (!OPENROUTER_API_KEY) throw new Error("Missing OPENROUTER_API_KEY");

  const prompt = `You are an expert technical writer. Write a comprehensive, well-structured blog post in valid HTML using <h1>, <h2>, <p>, <ul>, <li>, and <blockquote>. No <script> tags, no external scripts, no inline event handlers.

Use the following two recent news items as primary sources. Synthesize insights and add actionable context for developers, focusing on technology and cybersecurity implications for privacy-first sharing platforms like AnonShare.

Sources:\n${articles
    .map((a, i) => `(${i + 1}) Title: ${a.title}\nDesc: ${a.description || ""}\nURL: ${a.url}\nPublished: ${a.publishedAt || ""}\nSource: ${a.source?.name || ""}`)
    .join("\n\n")}\n\nStrict formatting requirements:\n- Length: roughly 900–1,300 words (aim for ~1,100).\n- Start with a single <h1> title summarizing the theme.\n- Include 3–4 <h2> sections (context, implications, best practices, and conclusion).\n- Use 8–14 short <p> paragraphs total; keep paragraphs concise.\n- Include one <ul> with 4–7 actionable takeaways for practitioners.\n- End with <h2>References</h2> and a <ul> of <a> links to the sources.\n- Do not include <html> or <body>; return an HTML fragment only.\n`;

  const messages = [
    { role: "system", content: "You write production-ready HTML blog posts with strict formatting and safety." },
  ...
  const res = await fetch(endpoint, {
    method: "POST",
    headers,
    body: JSON.stringify({ model, messages, temperature: 0.7, max_tokens: 2800 }),
  });
  if (!res.ok) {
...
serve(async (req) => {
  ...
  try {
    ...
    if (!OPENROUTER_API_KEY) {
      return new Response(JSON.stringify({ error: "Missing OPENROUTER_API_KEY" }), { headers: corsHeaders, status: 400 });
    }
    ...
    // 3) Generate blog HTML via OpenRouter
    const html = await callDeepSeek(null, articles);
